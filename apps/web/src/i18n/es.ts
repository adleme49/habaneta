// Spanish translations. Partial coverage for now — any key not
// present falls through to English via i18next's default behavior.
// Expand as needed.

import { Translations } from './en';

export const es: Translations = {
  common: {
    close: 'Cerrar',
    cancel: 'Cancelar',
    save: 'Guardar',
    delete: 'Borrar',
    confirm: 'Confirmar',
    backToEditor: '← volver al editor',
  },
  nav: {
    appTitle: 'Habaneta',
    library: 'Biblioteca →',
  },
  browser: {
    tiles: 'LOZAS',
    border: 'BORDES',
    selectCategory: 'Selecciona una categoría →',
    expand: 'Expandir buscador',
    collapse: 'Colapsar buscador',
    verticalLabel: 'Lozas',
  },
  editor: {
    undo: 'Deshacer',
    redo: 'Rehacer',
    selectTilePrompt: 'Selecciona una loza para editar',
    saveToRecents: 'Salvar a recientes',
    alreadyInRecents: 'Ya está en recientes',
    resetColors: 'Restablecer colores',
    resetColorsNoChanges: 'Sin cambios que restablecer',
    presets: 'Presets',
    saveAsPreset: 'Guardar como preset',
    newPreset: 'Nuevo preset',
    newPresetTitle: 'Nuevo preset',
    renamePreset: 'Renombrar {{name}}',
    renamePresetTitle: 'Renombrar preset',
    presetName: 'Nombre',
    presetNamePlaceholder: 'Nombre del preset…',
    noPresetsYet: 'Aún no hay presets.',
    applyPreset: 'Aplicar "{{name}}"',
    deletePreset: 'Borrar {{name}}',
    myColors: 'Mis colores',
    myColorsEmpty: 'Los colores guardados aparecerán aquí.',
    addCurrent: 'Añadir',
    addCurrentColor: 'Guardar el color actual en Mis colores',
    removeSavedColor: 'Quitar de Mis colores',
    palette: {
      expand: 'Expandir paleta',
      collapse: 'Colapsar paleta',
      hexPlaceholder: '#rrggbb',
      hexAriaLabel: 'Valor hex del color',
      openPicker: 'Abrir selector de color',
      selectColor: 'Seleccionar {{color}}',
      shades: 'Tonos',
      shadeLabel: 'Tono {{color}}',
    },
  },
  preview: {
    recentsEmpty: 'Las lozas elegidas aparecerán aquí.',
    roleFloor: 'Loza',
    roleBorder: 'Cenefa',
    onGrid: 'En la cuadrícula',
    removeRecent: 'Quitar de recientes',
    gridSize: 'Tamaño',
    gridPattern: 'Patrón',
    gridPatterns: {
      auto: 'Auto',
      simple: 'Simple',
      rotated: 'Rotado',
      pinwheel: 'Molinillo',
      mirrorX: 'Espejo ↕',
      mirrorY: 'Espejo ↔',
      checkerboard: 'Damero',
    },
    moreRows: 'Más filas',
    fewerRows: 'Menos filas',
    gallery: 'Galería',
    environment: 'Ambiente',
    share: 'Compartir',
    linkCopied: 'Enlace copiado',
    shareFallback: 'Portapapeles bloqueado — copia esta URL:',
    shareRestored: 'Diseño restaurado',
    sharePartial: 'Restaurado parcialmente — faltan {{count}} loza(s)',
    export: 'Exportar',
    exporting: 'Exportando…',
    exported: 'Diseño exportado',
    exportFailed: 'Error al exportar: {{error}}',
  },
  ambient: {
    scene: 'Escena',
    bathroom: 'Baño',
    kitchen: 'Cocina',
  },
  gallery: {
    title: 'Galería',
    next: 'Siguiente',
    previous: 'Anterior',
  },
  library: {
    pageTitle: 'Habaneta · Biblioteca',
    counter: '{{count}} lozas',
    counterSplit: '({{builtin}} incluidas · {{user}} tuyas)',
    searchPlaceholder: 'Buscar lozas…  (presiona / para enfocar)',
    searchClear: 'Limpiar',
    noMatch: 'No hay lozas que coincidan.',
    loading: 'Cargando catálogo…',
    failed: 'Error al cargar el catálogo: {{error}}',
    filters: {
      all: 'Todas',
      floors: 'Pisos',
      borders: 'Bordes',
      mine: 'Mías',
    },
    columns: {
      name: 'Nombre',
      family: 'Familia',
      kind: 'Tipo',
      layers: 'Capas',
      source: 'Origen',
      id: 'ID',
    },
    upload: {
      trigger: 'Subir loza',
      title: 'Subir una loza',
      description:
        'Arrastra un SVG con formas class="colora stN". Las capas se detectan automáticamente.',
      svgFile: 'Archivo SVG',
      name: 'Nombre',
      namePlaceholder: 'ej. Patrón floral 1',
      family: 'Familia',
      kind: 'Tipo',
      kindFloor: 'Piso',
      kindBorder: 'Borde',
      preview: 'Vista previa',
      previewEmpty: 'Elige un archivo para previsualizar',
      pickFilePrompt: 'Arrastra un SVG aquí, o haz clic para buscar',
      classHint: 'Las formas deben usar class="colora stN"',
      layersDetected: '{{count}} capas detectadas',
      saveTile: 'Guardar loza',
      saving: 'Guardando…',
      saveFailed: 'Error al guardar: {{error}}',
      quotaHint: 'Intenta exportar y borrar subidas antiguas primero.',
    },
    imageImport: {
      trigger: 'Importar imagen',
      title: 'Importar loza desde imagen',
      description:
        'Elige una foto de una loza real. Los colores se analizarán y convertirán en capas recoloreables.',
      imageFile: 'Archivo de imagen',
      layers: 'Capas',
      analyze: 'Analizar imagen',
      uploading: 'Subiendo…',
      queued: 'En cola…',
      processing: 'Analizando…',
      denoise: 'Reducción de ruido',
      contour: 'Detectar contornos',
      showAdvanced: 'Avanzado ▾',
      hideAdvanced: 'Avanzado ▴',
      targetPx: 'Resolución objetivo',
      contourSensitivity: 'Sensibilidad del contorno',
      contourMaxThickness: 'Grosor máximo del contorno (px)',
      reanalyze: 'Re-analizar',
      swatchesHint: 'Haz clic en un color para reemplazarlo:',
      backendDown: 'El backend de importación no responde. Inícialo y reintenta.',
      retry: 'Reintentar',
      lighting: 'Iluminación',
      lightingHint:
        'Compensa luz tenue, sol fuerte o iluminación cálida de interior. Los colores fijados se mantienen.',
      lightingReset: 'Restablecer',
      exposure: 'Exposición',
      warmth: 'Calidez',
      saturation: 'Saturación',
      auto: 'Auto',
      layersCount: '{{count}} capas',
      layersAutoCount: '{{count}} capas (auto)',
      appliedPrefix: 'aplicado:',
      original: 'Original',
      result: 'Resultado',
      pickPrompt: 'Elige una imagen para empezar',
      layersDetected: '{{count}} capas detectadas',
      saveTile: 'Guardar loza',
      symmetry: 'Simetría',
      symmetryNone: 'Ninguna',
      symmetry2fold: '2 ejes (180°)',
      symmetry4fold: '4 ejes (90°)',
      autoCrop: 'Recortar bordes automáticamente',
      autoLevels: 'Equilibrar colores automáticamente',
      similarColorsHint: 'Sugerencia: hay colores similares — prueba con menos capas.',
      resolution: 'Resolución',
      resolutionHint: 'Mayor = bordes más nítidos, análisis más lento.',
    },
    importExport: {
      export: 'Exportar',
      import: 'Importar',
      exported: 'Exportada {{count}} loza',
      exportedPlural: 'Exportadas {{count}} lozas',
      imported: 'Importadas {{count}}',
      skipped: ', omitidas {{count}}',
      errors: ', {{count}} errores',
      exportFailed: 'Error al exportar: {{error}}',
      importFailed: 'Error al importar: {{error}}',
    },
    detail: {
      family: 'Familia',
      kind: 'Tipo',
      source: 'Origen',
      layers: 'Capas',
      gridPatterns: 'Patrones de rejilla',
      cornerVariant: 'Variante de esquina',
      cornerVariantYes: 'sí',
      defaultPalette: 'Paleta por defecto',
      openInEditor: 'Abrir en editor',
    },
  },
};
